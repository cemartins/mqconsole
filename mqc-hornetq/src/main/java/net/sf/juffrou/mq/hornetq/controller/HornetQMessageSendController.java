...
public class HornetQMessageSendController implements MessageSendController {
	...
	protected static final Logger LOG = LoggerFactory.getLogger(HornetQMessageSendController.class);
	...
	public void sendMessage(MessageSendPresenter presenter, MessageDescriptor messageDescriptor, String queueNameSend, String queueNameReceive) {
		...
		if(sendMessageId == null) {
			...
			try {
				MessageDescriptor replyMessageDescriptor = HornetQMessageDescriptorHelper.createMessageDescriptor(replyMessage);
			...
			HornetQMessageReceivingTask task = new HornetQMessageReceivingTask(handler, jmsTemplate, queueNameReceive, brokerTimeout,	sendMessageId, queueNameSend);
	...
	private static class MyMessageProducer implements ProducerCallback<String> {
		...
		public String doInJms(Session session, MessageProducer producer) throws JMSException {
			...
			HornetQMessageDescriptorHelper.setMessageHeaders(message, messageDescriptor);
